...
b.	Identify the Front Door paths.
  ...
  a. income -> health
  b. income -> hcare -> health
    
c.	Identify th.e Back Door paths. 

  a. income <- race -> health
  b. income <- occupation -> health
  c. income <- age -> health
  d. income <- race -> occupation -> health
  e. income <- occupation <- race -> health  
...
a.	Write down all the paths in the diagram from Lockdown to Recession. To make our lives simpler (there are a lot of paths in this diagram), ignore any path that goes through Stimulus.
  ...
  a. lock -> unemp -> rec
  b. lock -> rec
  c. lock -> unemp <- priorec -> rec
  d. lock <- priorec -> unemp -> rec
  e. lock <- priorec ->  rec
...
c.	What would happen if we controlled for unemployment? 
  ...
  It will show an association between Lockdown and Stimulus that was not previously there. I wonder if doing so would alter the already existent relations between PriorEcon and Lockdown, and PriorEcon and Stimulus.
...
a.	What potential back doors might remain between the tariffs and the opinion of the United States?
  ...
  tariff -> export <- US_partnership -> higher_opinion
...
b.	If someone told you that they performed this study as described, and found that increased exports had no effect on opinion of the US, would you believe the result? Why or why not? 
  ...
  I would not beleive it because not all backdoors are closed, so there are alternative explanations that must be discarded first. 
